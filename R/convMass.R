...
convMass <- function(reading,
  ...
  cat(paste0('Masa corregida: ', corrected, ' [', units, ']\n'))
  return(corrected)
}
...
# Ejemplo Andrés Castillo
convMass(reading = 12.4835,
         calibCert = calibCert(massSTD = c(5, 20), #[g]
                               indError = c(-0.2, -0.1), #[mg]
                               uncert = c(0.4, 0.41),
                               d = 0.1, #[mg]
                               units = c('g', 'mg', 'mg', 'mg')))

convMass(reading = 12483.5, units = 'mg',
         calibCert = calibCert(massSTD = c(5, 20), #[g]
                               indError = c(-0.2, -0.1), #[mg]
                               uncert = c(0.4, 0.41),
                               d = 0.1, #[mg]
