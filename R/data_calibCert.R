#' Calibration data for a balance Mettler Toledo XPE 204
#'
#' An object of class \code{calibCert()} with the calibration information of a balance
#' Mettler Toledo XPE 204, performed by the Mass Laboratory at the Instituto Nacional de Metrologia de
#' Colombia (2021-03-18).
#'
#' @format An object of class \code{calibCert()}
#' @source
#'   Mass Laboratory - Instituto Nacional de Metrologia de Colombia. CALIBRATION CERTIFICATE No. 5143.
"MT.XPE.204"

#' Calibration data for a balance Mettler Toledo XP 2002
#'
#' An object of class \code{calibCert()} with the calibration information of a balance
#' Mettler Toledo XP 2002, performed by the Mass Laboratory at the Instituto Nacional de Metrologia de
#' Colombia (2021-03-12).
#'
#' @format An object of class \code{calibCert()}
#' @source
#'   Mass Laboratory - Instituto Nacional de Metrologia de Colombia. CALIBRATION CERTIFICATE No. 5142.
"MT.XP.2002"

#' Minimal \code{calibCert()} object
#'
#' An object of class \code{calibCert()} with the minimal calibration information of a hypothetical balance
#' calibrated using only two points.
#'
#' @format An object of class \code{calibCert()}
"minimalCert"
